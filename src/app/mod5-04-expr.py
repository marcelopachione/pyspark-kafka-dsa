...
docker exec -it dsa-pyspark-master /opt/spark/bin/spark-submit \
   ...
   --deploy-mode client \
   /opt/spark/apps/mod5-04-expr.py
"""
...
spark = SparkSession.builder.appName('Script04').getOrCreate()
...
df2 = df.withColumn("genero", expr("CASE WHEN genero = 'M' THEN 'Masculino' " + "WHEN genero = 'F' THEN 'Feminino' ELSE 'Desconhecido' END"))
